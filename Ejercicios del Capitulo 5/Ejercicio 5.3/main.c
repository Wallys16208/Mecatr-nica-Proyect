...
int main(void)
{
    ...
    *IX += 1;          /* Z[2] se vuelve a modificar, ahora vale 12. */
    ...
    /* No debemos mover el puntero fuera del arreglo */
    /* IX = IX + 4; */ /* Esto causaría un error al intentar acceder a memoria fuera del arreglo */
    /* Y = *IX; */
    /* printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z[4] = %d",
           X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]); */

    IX = &X;            /* IX apunta a la variable entera X. */
    /* IX = IX + 1; */ /* Esto no es seguro, no se debe acceder a memoria fuera del control del programa */
    /* X = *IX; */
    printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z[4] = %d",
           X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
